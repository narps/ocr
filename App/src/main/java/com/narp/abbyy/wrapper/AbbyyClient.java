package com.narp.abbyy.wrapper;
...
import com.ocr.ClientSettings;
import com.ocr.CmdLineOptions;

public class AbbyyClient {

	public static String ocrIt(String[] args) {
		...
		try {
			if (mode.equalsIgnoreCase("help")) {
				...
				result = performRecognition(argList);
	...
	private static void displayRecognizeHelp() {
		System.out.println("Recognize single or multipage documents.\n" + "\n" + "Usage:\n"
	 ...
	 */
	private static String performRecognition(Vector<String> argList) throws Exception {
		...
		Task task = null;
		if (argList.size() == 1) {
			System.out.println("Uploading file.. " + settings.toString() + "  argList.elementAt(0) " + argList.elementAt(0));
	...
	private static void displayBusCardHelp() {
		System.out.println("Recognize single business card.\n" + "\n" + "Usage:\n"
	 ...
	 * vCard, csv or xml
	 */
	private static void performBusinessCardRecognition(Vector<String> argList) throws Exception {
		String language = CmdLineOptions.extractRecognitionLanguage(argList);
		String outputPath = argList.lastElement();
		argList.remove(argList.size() - 1);
		// argList now contains list of source images to process
		...
		Task task = restClient.processBusinessCard(argList.elementAt(0), settings);
	 ...
	 */
	private static String performReceiptProcessing(Vector<String> argList) throws Exception {
		...
		String out = waitAndDownloadResult(task, outputPath);
		return out;
	}

	private static void displayTextFieldHelp() {
		System.out.println("Recognize printed or handprinted text field.\n" + "\n" + "Usage:\n"
	...
	private static void performTextFieldRecognition(Vector<String> argList) throws Exception {
		String language = CmdLineOptions.extractRecognitionLanguage(argList);
	...
	private static void displayBarcodeHelp() {
		System.out.println("Recognize barcode.\n" + "\n" + "Usage:\n" + "java TestApp barcode <file> <output file>\n"
				+ "\n" + "Examples:\n" + "java TestApp barcode image.tif result.xml\n");
	}

	private static void performBarcodeRecognition(Vector<String> argList) throws Exception {
		String outputPath = argList.lastElement();
		argList.remove(argList.size() - 1);
		// argList now contains list of source images to process
		...
		System.out.println("Uploading..");
		Task task = restClient.processBarcodeField(argList.elementAt(0), settings);

		waitAndDownloadResult(task, outputPath);
	}

	private static void displayProcessFieldsHelp() {
		System.out.println("Process different snippets in one- or multipage document.\n" + "\n" + "Usage:\n"
	 ...
	 */
	private static void performFieldsRecognition(Vector<String> argList) throws Exception {
	...
	private static void displayProcessMrzHelp() {
		System.out.println(
				"Recognize Machine-Readable Zones of official documents\n" + "Both 2 and 3-line MRZ are supported."
	...
	private static String performMrzRecognition(Vector<String> argList) throws Exception {
		String outputPath = argList.lastElement();
		argList.remove(argList.size() - 1);
		// argList now contains list of source images to process
	 ...
	 */
	private static String waitAndDownloadResult(Task task, String outputPath) throws Exception {
		task = waitForCompletion(task);
		String output = null;
		if (task.Status == Task.TaskStatus.Completed) {
			System.out.println("Downloading..");
			output = restClient.downloadResult(task, outputPath);
			//processCSV(outputPath);
	...
	private static BusCardSettings.OutputFormat bcrOutputFormatByFileExt(String filePath) {
		int extIndex = filePath.lastIndexOf('.');
		if (extIndex < 0) {
			System.out.println("No file extension specified. vCard will be used as output format.");
		...
		System.out.println("Invalid file extension. vCard will be used as output format.");
		return BusCardSettings.OutputFormat.vCard;
	}
